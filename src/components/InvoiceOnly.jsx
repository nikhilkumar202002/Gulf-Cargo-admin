...
import React, { useEffect, useMemo, useRef, useState } from "react"; // ⬅️ added useRef
...
const getParam = (search, key) => new URLSearchParams(search).get(key); // ⬅️
...
const flattenBoxesToItems = (boxes) => {
  ...
  for (const k of ordered) {
    ...
    for (const it of list) {
      const rawBox = it?.box_number ?? it?.box_no ?? k;
...
export default function InvoiceView({ shipment: injected = null, modal = false }) {
  ...
  // ⬅️ NEW: ref to the printable/downloadable sheet
  ...
  const getName = (p, side, shipment) =>
    ...
    "";
  const getAddress = (p, side, shipment, pickFn) =>
    p?.address ||
    pickFn?.(shipment?.[side], ["address"], "") ||
    pickFn?.(
      shipment,
      side === "sender"
  ...
  const getPhone = (p, side, shipment, pickFn) =>
    p?.phones ||
    pickFn?.(shipment?.[side], ["contact_number", "whatsapp_number"], "") ||
    pickFn?.(
      shipment,
      side === "sender"
  ...
  const getPincode = (p, side, shipment, pickFn) =>
    p?.pincode ||
    pickFn?.(shipment?.[side], ["pincode", "pin", "zip", "zipcode", "postal_code"], "") ||
    pickFn?.(
      shipment,
  ...
  // ⬅️ NEW: create a one-page A4 PDF from the visible sheet
  const handleDownloadA4 = async () => {
    const node = sheetRef.current || document.getElementById("invoice-sheet");
    if (!node) return;

    // Ensure current size is captured at good quality
    ...
    pdf.addImage(imgData, "PNG", offX, offY, imgW, imgH, undefined, "FAST");

    const name = `invoice_${shipment?.booking_no || shipment?.invoice_no || shipment?.track_code || id}.pdf`;
    pdf.save(name.replace(/\s+/g, "_"));
  };

  // ⬅️ NEW: support auto-download via ?download=1
  useEffect(() => {
    if (!shipment) return;
    if (getParam(location.search, "download") === "1") {
      const t = setTimeout(() => handleDownloadA4(), 150);
      return () => clearTimeout(t);
    }
  }, [shipment, location.search]);
        ...
        <div className="sticky top-0 z-10 border-b bg-white print:hidden">
          <div className="mx-auto max-w-5xl px-4 py-3 flex items-center gap-2">
            ...
            <div className="ml-auto flex items-center gap-2">
              <button
                onClick={handleDownloadA4} // ⬅️ new
                className="rounded-lg bg-[#262262] px-4 py-1.5 text-sm font-semibold text-white hover:bg-[#161436]"
              >
                Download A4 PDF
              </button>
              <button
...
>
          ...
          <div className="px-1 pt-1">
            <div className="grid grid-cols-3 items-start ">
              ...
              <div className="invoice-qrcode flex items-center justify-center">
                <img
                  src={buildQrUrl(buildTrackUrl(shipment), 160)}
  ...
  <tr>
    <th className="w-12 border border-slate-200 px-1  text-left uppercase tracking-wider">Sl.No</th>
    <th className="border-t border-b border-slate-200 px-1  text-left uppercase tracking-wider">Items</th>
    <th className="border border-slate-200 px-1  text-right text-[11px] font-semibold uppercase tracking-wider">Box No.</th>
    <th className="border border-slate-200 px-1  text-right uppercase tracking-wider">Qty</th>
    {/* NEW */}
  ...
  {colA.map((it, idx) => (
    <tr key={`A-${idx}`} className="align-top">
      <td className="border-x border-b border-slate-200 px-1 ">{it ? it.idx : ""}</td>
      <td className="border-b border-slate-200 px-1  ">{it ? (it.name || "—") : <span className="opacity-0">pad</span>}</td>
      <td className="border-x border-b border-slate-200 px-1  text-right text-sm text-slate-900">{it ? (it.boxLabel || "—") : ""}</td>
      <td className="border-x border-b border-slate-200 px-1  text-right text-sm text-slate-900">{it ? it.qty || "—" : ""}</td>
  ...
  <tr>
    ...
    <th className="border-t border-b border-slate-200 px-1  text-left text-[11px] font-semibold uppercase tracking-wider ">Items</th>
    <th className="border border-slate-200 px-1  text-right text-[11px] font-semibold uppercase tracking-wider ">Box No.</th>
    <th className="border border-slate-200 px-1  text-right text-[11px] font-semibold uppercase tracking-wider ">Qty</th>
  ...
  {colB.map((it, idx) => (
    <tr key={`B-${idx}`} className="align-top">
      <td className="border-x border-b border-slate-200 px-1 ">{it ? it.idx : ""}</td>
      <td className="border-b border-slate-200 ppx-1 ">{it ? (it.name || "—") : <span className="opacity-0">pad</span>}</td>
      <td className="border-x border-b border-slate-200 px-1  text-right">{it ? (it.boxLabel || "—") : ""}</td>
      <td className="border-x border-b border-slate-200 px-1  text-right ">{it ? it.qty || "—" : ""}</td>
