...
const SkelLine = ({ w = 100, h = 14, className = "" }) => (
  <span
    ...
    style={{ width: typeof w === "number" ? `${w}px` : w, height: typeof h === "number" ? `${h}px` : h, display: "inline-block", borderRadius: 6 }}
    aria-hidden="true"
  />
);
...
const Card = ({ title, children, loading, rows = 4 }) => (
  <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-0">
    <div className="px-5 py-4 border-b border-gray-100">
      <h3 className="text-base font-semibold text-gray-800">
        {loading ? <SkelLine w={140} h={16} /> : title}
      </h3>
    </div>
    <div className="p-4 sm:p-5">
      {children ?? null}
      {/* Optional skeleton rows if you want them on the whole card */}
      {/* {loading && (
        <div className="grid gap-3">
          {Array.from({ length: rows }).map((_, i) => (
            <div key={i} className="flex items-center justify-between py-2">
              <div className="flex items-center gap-3">
                <span className="w-9 h-9 rounded-lg skel" />
                <SkelLine w={160} h={14} />
              </div>
              <SkelLine w={60} h={14} />
            </div>
...
const Row = ({ Icon, label, value, iconBg = "bg-gray-100", iconColor = "text-gray-700", loading }) => (
  <div className="flex items-center justify-between py-3">
    <div className="flex items-center gap-3">
      <div className={`w-9 h-9 rounded-lg grid place-items-center ${loading ? "skel" : iconBg}`}>
        {!loading && <Icon className={`${iconColor}`} />}
      </div>
      <span className="text-gray-800">
        {loading ? <SkelLine w={140} h={14} /> : label}
      </span>
    </div>
    <span className="font-semibold text-gray-900">
      {loading ? <SkelLine w={40} h={14} /> : value}
    </span>
  </div>
);
    ...
    <section className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-8">
          ...
          <KPI value={dummy.exceptionsOpen} label="Exceptions Open" Icon={FaExclamationTriangle} loading={loading} />
          <KPI value={fmtMoney(dummy.revenue)} label="Revenue (Filtered)" Icon={FaDollarSign} sublabel="Gross Margin 31%" loading={loading} />
          ...
          <KPI value={dummy.outForDelivery} label="Out for Delivery" Icon={FaTruck} loading={loading} />
          <KPI value={dummy.pendingDispatch} label="Pending Dispatch" Icon={FaRegClock} loading={loading} />
        </div>

        {/* BELOW: Rest */}
        <h2 className="text-lg sm:text-xl font-semibold text-gray-800 mb-4">
          {loading ? <SkelLine w={200} h={18} /> : "Operations Overview"}
        </h2>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <Card title="Staff Attendance">
            <div className="grid">
              <Row Icon={FaUserCheck} label="Present" value={dummy.present} iconBg="bg-green-100" iconColor="text-green-700" loading={loading} />
              <Row Icon={FaUserTimes} label="Absent" value={dummy.absent} iconBg="bg-rose-100" iconColor="text-rose-700" loading={loading} />
              <Row Icon={FaUserMinus} label="Partial" value={dummy.partial} iconBg="bg-yellow-100" iconColor="text-yellow-700" loading={loading} />
              <Row Icon={FaExchangeAlt} label="Moving Pending" value={dummy.movingPending} iconBg="bg-fuchsia-100" iconColor="text-fuchsia-700" loading={loading} />
            </div>
          </Card>

          <Card title="Cargo & Movement Overview">
            <div className="grid">
              <Row Icon={FaInbox} label="Receiver" value={live.receiver} iconBg="bg-blue-100" iconColor="text-blue-700" loading={loading} />
              <Row Icon={FaTruck} label="Out for Delivery" value={dummy.outForDelivery} iconBg="bg-purple-100" iconColor="text-purple-700" loading={loading} />
              <Row Icon={FaRegClock} label="Waiting for Clearance" value={dummy.waitingClearance} iconBg="bg-orange-100" iconColor="text-orange-700" loading={loading} />
              <Row Icon={FaInbox} label="Enquiries Collected" value={dummy.enquiriesCollected} iconBg="bg-cyan-100" iconColor="text-cyan-700" loading={loading} />
            </div>
          </Card>
        </div>
      </div>
