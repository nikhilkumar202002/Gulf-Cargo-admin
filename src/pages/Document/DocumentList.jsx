...
const DocumentTypeList = () => {
  ...
  const fetchDocumentTypes = async () => {
    try {
      setLoading(true);
      const response = await getDocumentTypes(token); // Call the API function here

      if (response.success) {
        setDocumentTypes(response.data || []);
      } else {
        setDocumentTypes([]);
        setError(response.message || "Failed to fetch document types");
      }
    } catch (err) {
            ...
            <tbody>
              {documentTypes.map((doc) => (
                <tr key={doc.id} className="border-b hover:bg-gray-50 transition-colors">
                  <td className="py-4 px-6 font-medium text-gray-900">{doc.document_name}</td>
                  <td className="py-4 px-6">
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${doc.status === "Active" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                      {doc.status}
                    </span>
                  </td>
